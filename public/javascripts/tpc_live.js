...
function PMT_setcolour(json_result, timestamp){
    ...
    if(got_updates == false){
        ...
        if(no_data_counter >= max_data_misses){
            ...
            stop_intervals();
            no_data_counter = 0;
            svgObject1.getElementById("str_legend_tme").textContent = "it seems no live data is available";
        } else {
    ...
    for (var i of Object.keys(pmt_rates)) {
        ...
        if(pmt_rate >= 0){
            ...
            var rgb_g = 1 - color_scheme(pmt_rate);
            var rgb_b = 0//(255 - pmt_rates[i] * color_scaler);
        ...
        var rgb_factor = 1/ (rgb_r**2 + rgb_g**2 + rgb_b**2)**.5 * 255
        
        var rgb_r = rgb_r * rgb_factor
        var rgb_g = rgb_g * rgb_factor
        var rgb_b = rgb_b * rgb_factor
...
function get_TPC_data(timestamp = false) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            var t_end = Date.now();
            if(setcolor_return != false){
                if(timestamp != false){
                    console.log("updated pmts in " + (t_end - t_start) + " ms for " + timestamp)
                } else {
                    console.log("updated pmts in " + (t_end - t_start) + " ms")
                }
            } else {
                if(timestamp != false){
                    console.log("database returned no data ("+no_data_counter+") for timestamp " + timestamp);
                } else {
                    console.log("database returned no data ("+no_data_counter+")");
                }
            } 
        }
    };
    
    str_url = "/tpc/updates"+unixtimestamp;
    xmlhttp.open("GET", str_url, true);
    xmlhttp.send();
...
function history_draw(){
    ...
    if(pmts == false){
        ...
        str_url = "/tpc/get_history?str_pmts=" + pmts +
